            ...
            while video_stream.isActive() and visualizer.isActive():
                ...
                if not (model.config.USE_TRACKER and track):
                    if model.config.SPLIT_MODEL:
                        ...
                        # default session
                        frame = video_stream.read()
                        output_dict = sess.run(tensor_dict, feed_dict={image_tensor: video_stream.expanded()})
                ...
                # Tracking
                    ...
                    if first_track:
                        ...
                        for box in boxes[~np.all(boxes == 0, axis=1)]:
                                tracker.init(conv_detect2track(box,video_stream.real_width, video_stream.real_height), tracker_frame)
                                trackers.append(tracker)
                        first_track = False

                    for idx,tracker in enumerate(trackers):
                        tracker_box = tracker.update(frame)
                        tracker_boxes[idx,:] = conv_track2detect(tracker_box, video_stream.real_width, video_stream.real_height)
                    visualizer.visualize_objectdetection(frame,tracker_boxes,classes,scores,masks,category_index,
                                                        fps._glob_numFrames,fps.fps_local())
                    tracker_counter += 1
                    #tracker_frame = frame
